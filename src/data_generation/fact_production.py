    ...
    for tag in tage:
        ...
        for linie_id, linie in LINIEN.items():
            ...
            for schicht_name, schicht in SCHICHTEN.items():
                ...
                auslastung = (
                    basis_auslastung * schicht_faktor * wochenend_faktor + zufall
                )
                auslastung = np.clip(auslastung, 40, 98)

                # Menge berechnen
                menge_gut = (
                    basis_menge * effizienz * (auslastung / 100) * schicht_faktor
                )
                ...
                laufzeit_minuten = int(
                    480 * (auslastung / 100) * np.random.uniform(0.9, 1.0)
                )
